...
// Importsconst cds = require("@sap/cds");/** * The service implementation with all service handlers */
module.exports = cds.service.impl(async function () { 
    // Define constants for the Risk and BusinessPartners entities from the risk-service.cds file 
    const { Risks, BusinessPartners } = this.entities; 
...
//### BEGIN OF INSERT // connect to remote service 
...
//### BEGIN OF INSERT /** * Event-handler on risks. * Retrieve BusinessPartner data from the external API */ 
    ...
    if ( 
        ...
        /* Instead of carrying out the expand, issue a separate request forneach business partner This code could be optimized, instead of having n requests for n business partners, just one bulk request could be created */ 
        try { 
            res = await next(); 
